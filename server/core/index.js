...
const EXTRACTION_MAX_CONCURRENCY = 2;
const EXCERPT_MAX_DEFAULT = 1500;
const EXCERPT_MAX_LIMIT = 3000;
const FILE_LIST_PREVIEW_LIMIT = 1500;
const snapshotDebounceMs = 600;
const extractionQueueState = {
  processing: false,
  timer: null,
  pending: new Set(),
  running: new Set(),
...
const topicSnapshotTimers = new Map();
...
db.exec(`
  CREATE INDEX IF NOT EXISTS idx_files_topic_role_order ON files(topic_id, role, sort_order, created_at);
  CREATE INDEX IF NOT EXISTS idx_files_topic_status ON files(topic_id, extraction_status, updated_at);
  CREATE INDEX IF NOT EXISTS idx_notes_topic_created ON notes(topic_id, created_at);
  CREATE INDEX IF NOT EXISTS idx_discussion_inputs_topic_created ON discussion_inputs(topic_id, created_at);
  CREATE INDEX IF NOT EXISTS idx_meeting_messages_topic_created ON meeting_messages(topic_id, created_at);
  CREATE INDEX IF NOT EXISTS idx_directions_topic_order ON discussion_directions(topic_id, sort_order, created_at);
  CREATE INDEX IF NOT EXISTS idx_activities_topic_created ON activities(topic_id, created_at);
  CREATE INDEX IF NOT EXISTS idx_background_tasks_topic_status_created ON background_tasks(topic_id, status, created_at);
`);
...
function rowToFile(row, includeContent = false) {
  const meta = rowToFileMeta(row);
  if (!meta) return null;
  if (!includeContent) return meta;
  const content = rowToFileContent(row);
  return {
    ...meta,
...
function normalizeFileListRow(row) {
  const file = rowToFile(row, true);
  if (!file) return null;
  return {
    ...file,
...
function rowToFileMetaLegacy(row) {
  const file = rowToFile(row, true);
  if (!file) return null;
  const meta = rowToFileMeta(row);
  return {
...
function compactFilePromptLine(file, maxChars = 900) {
  ...
  const preview = compactPromptText(file.summary || "", Math.min(file.kind === "image" ? 140 : 200, maxChars));
  const previewText = preview ? `\n  预览：${preview}` : "";
  return `- 文件名：${file.originalName}｜${file.kind}${status}\n  摘要：${summary || "暂无可读摘要。"}${previewText}`;
}

function rowToFileLegacy(row, includeContent = true) {
  return rowToFile(row, includeContent);
}
